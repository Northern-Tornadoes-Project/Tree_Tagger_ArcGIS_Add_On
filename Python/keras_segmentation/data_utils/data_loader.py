...
def get_image_array(image_input,
    ...
    if imgNorm == "sub_and_divide":
        ...
        #the mean pixel values of all images in image net, most keras segmentation model starting weight were trained on
        img[:, :, 0] -= 103.939
        img[:, :, 1] -= 116.779
        img[:, :, 2] -= 123.68
        #bgr to rgb
...
def image_segmentation_generator(images_path, segs_path, batch_size,
    ...
    # img_seg_pairs_data = []
    #
    # for img, seg in img_seg_pairs:
    #     img_seg_pairs_data.append((cv2.imread(img, cv2.IMREAD_COLOR), cv2.imread(seg, cv2.IMREAD_COLOR)))
