...
std::vector<std::vector<std::array<float, 3>>> averageDirections(std::vector<std::array<float, 5>> lines, const float gridSize, const int x, const int y) {

    std::vector<std::vector<std::array<float, 3>>> averageDirectionsGrid;
    for (int i = 0; i < y; i++) {
        averageDirectionsGrid.push_back({});
        for (int j = 0; j < x; j++) {
            averageDirectionsGrid.at(i).push_back({1e10f, 1e10f, 0.0f});
        }
    }
    ...
    for (int gridRowIdx = 0; gridRowIdx < y; gridRowIdx++) {
        for (int gridColIdx = 0; gridColIdx < x; gridColIdx++) {
            ...
            if (numOfLines > 3 && averageMagnitude > 0.6666666f) {
                ...
                averageDirectionsGrid.at(gridRowIdx).at(gridColIdx) = { averageVec[0] / averageMagnitude, averageVec[1] / averageMagnitude, 1.0f };
            }
        }
        ...
        for (int gridRowIdx = 0; gridRowIdx < y; gridRowIdx++) {
            for (int gridColIdx = 0; gridColIdx < x; gridColIdx++) {
                ...
                if (total >= minNumOfSurrounding) {
                    ...
                    averageDirectionsGrid.at(gridRowIdx).at(gridColIdx) = {medianVec[0] / medianMagnitude, medianVec[1] / medianMagnitude, 1.0f};
                }

            }
        }

        if (!found && minNumOfSurrounding == 5) break;

        if (minNumOfSurrounding > 5) minNumOfSurrounding--;
    }
