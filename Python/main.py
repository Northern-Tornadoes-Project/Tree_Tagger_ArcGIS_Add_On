...
try:
    ...
    directionGridSizes = [int(x) for x in argv[:3]]
...
for imgData in tqdm(imagesData, file=sys.stdout):
    ...
    for line in coordLines:
        if (imgData[1] < line[0] < (imgData[1] + image.shape[1] * imageScale[0]) and (imgData[2] - image.shape[0] * imageScale[1]) < line[1] < imgData[2]) or (imgData[1] < line[2] < (imgData[1] + image.shape[1] * imageScale[0]) and (imgData[2] - image.shape[0] * imageScale[1]) < line[3] < imgData[2]):
...
for size in directionGridSizes:
    ...
    grid_directions.append(FastLib.averageDirections(predicted_lines, (size/imageScale[0]), x, y))
...
for k in range(len(directionGridSizes)):
    ...
    for i in range(y):
        for j in range(x):
            ...
            if math.isnan(gridVec[0]) or math.isnan(gridVec[1]):
               continue
...
for line in predicted_lines:
    if math.sqrt((line[3] - line[1]) ** 2 + (line[2] - line[0]) ** 2) < (5.0/imageScale[0]):
        if line[2] == line[0]:
            line[2] += 1
        slope = float(line[3] - line[1]) / float(line[2] - line[0])
        scale = (5.0/imageScale[0]) / math.sqrt(1 + slope ** 2)
...
allLines = FastLib.joinLines(allLines, x, y, float(0.3), float(0.1), float(10), float(5), float(15))
