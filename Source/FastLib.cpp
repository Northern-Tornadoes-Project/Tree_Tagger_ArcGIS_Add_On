...
std::vector<std::array<float, 4>> joinLines(std::vector<std::array<float, 4>> lines, const int x, const int y, const float angleThreshold, const float directMergeAngleThreshold, float directMergeThreshold, float distThreshold, float minLineLength, float maxLineLength) {
    ...
    for (auto& l : lines) {
        if (l[0] >= 0) {
            const float d = distSquared(l[2] - l[0], l[3] - l[1]);
...
std::vector<std::vector<std::array<float, 3>>> medianDirections(std::vector<std::array<float, 5>> lines, const float gridSize, const int x, const int y, const int extrapolationReq, const int minNumOfLines) {

    std::vector<std::vector<std::array<float, 3>>> directionsGrid;
    for (int i = 0; i < y; i++) {
        directionsGrid.push_back({});
        for (int j = 0; j < x; j++) {
            directionsGrid.at(i).push_back({ 1e10f, 1e10f, -1.0f });
        }
    }
    ...
    for (int gridRowIdx = 0; gridRowIdx < y; gridRowIdx++) {
        for (int gridColIdx = 0; gridColIdx < x; gridColIdx++) {
            ...
            for (auto lineIdx : gridSquare) {
                ...
                //compute unit vector and add to sum
                const float magnitude = sqrtf(distSquared(lineVector[0], lineVector[1]));
                const float unitX = lineVector[0] / magnitude;
                const float unitY = lineVector[1] / magnitude;

                xDirections.push_back(unitX);
                yDirections.push_back(unitY);
            ...
            directionsGrid.at(gridRowIdx).at(gridColIdx) = { medianVec[0] / medianMagnitude, medianVec[1] / medianMagnitude, averageMagnitude };
...
std::vector<std::vector<std::array<float, 3>>> medoidDirections(std::vector<std::array<float, 5>> lines, const float gridSize, const int x, const int y, const int extrapolationReq, const int minNumOfLines) {
    ...
    for (int gridRowIdx = 0; gridRowIdx < y; gridRowIdx++) {
        for (int gridColIdx = 0; gridColIdx < x; gridColIdx++) {
            ...
            std::vector<std::array<float, 2>> unitVecs;
            std::map<int, int> indexMap;
            float minDisimilarity = std::numeric_limits<float>::max();
            std::array<float, 2> medoid;
            ...
            for (auto lineIdx : gridSquare) {
                ...
                unitVecs.push_back({ unitX, unitY });

                float angle = atan2f(unitY, unitX);
                angle = angle > 0 ? angle : 2 * PI + angle;

                angles.push_back(angle);
                indexMap.insert({ lineIdx, angles.size() - 1 });
            ...
            for (auto lineIdx : gridSquare) {
                ...
                int dataIdx1 = indexMap.find(lineIdx)->second;

                for (auto lineIdx2 : gridSquare) {
                    if (lineIdx == lineIdx2) continue;

                    int dataIdx2 = indexMap.find(lineIdx2)->second;

                    disimilarity += PI - fabs(fabs(angles[dataIdx1] - angles[dataIdx2]) - PI);
                }

                if (minDisimilarity > disimilarity) {
                    minDisimilarity = disimilarity;
                    medoid = unitVecs[dataIdx1];
                }
            }
            ...
            directionsGrid.at(gridRowIdx).at(gridColIdx) = { medoid[0], medoid[1], averageMagnitude};
...
PYBIND11_MODULE(FastLib, handle){
    ...
    handle.def("medianDirections", &medianDirections);
    handle.def("medoidDirections", &medoidDirections);
