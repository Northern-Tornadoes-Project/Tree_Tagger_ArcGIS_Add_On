'''
    ...
    Email: dbutt7@uwo.ca
    Date of last revision: Aug 26, 2022
'''
import treetag
from treetag import *
import math
...
try:
    ...
    image_scale = (round(float(argv[0]), 6), round(float(argv[1]), 6))

    top_left = (float(argv[2]), float(argv[3]))

    bottom_right = (float(argv[4]), float(argv[5]))

    is_polygon = bool(int(argv[6]))
...
try:
    model = treetag.TreeSegModel(os.path.dirname(os.path.realpath(__file__)) + "\\weights30.h5")
except Exception:
...
model = treetag.TreeDirectionModel(os.path.dirname(os.path.realpath(__file__)) + "\\weights9_.h5")
...
grid_vectors = build_vectors(direction_grid_parameters, top_left, bottom_right, predicted_lines, image_scale)
