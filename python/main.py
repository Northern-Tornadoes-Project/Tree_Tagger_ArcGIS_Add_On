...
mask = np.zeros([int(math.ceil(regionHeight / regionScale)), int(math.ceil((regionHeight / regionScale)))], np.uint8)
...
for imgData in tqdm(imagesData):
    ...
    if isPolygon:
        maskSection = mask[int(offset[1] / regionScale):int((offset[1] + image.shape[0]) / regionScale), int(offset[0] / regionScale):int((offset[0] + image.shape[1]) / regionScale)]
        maskSection = cv2.resize(maskSection, (image.shape[0], image.shape[1]), cv2.INTER_NEAREST)
        image = cv2.bitwise_and(image, image, mask=maskSection)

    original_image_shape = (image.shape[0], image.shape[1])
    y = int(max(original_image_shape[0] / 256.0, 1))
    x = int(max(original_image_shape[1] / 256.0, 1))
